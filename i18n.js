const translations = {
  en: {
    primaryNavigation: "Primary navigation",
    viewRevenueDetails: "View revenue details on the Orders page",
    viewExpenseDetails: "View expense details on the Expenses page",
    viewOrdersDetails: "View orders on the Orders page",
    currentBalanceSummary: "Current balance summary",
    visitLinkedIn: "Visit Sumayyah's LinkedIn profile",
    visitGithub: "Visit Sumayyah's GitHub profile",
    sendEmail: "Send an email to Sumayyah",
    editExpense: "Edit expense",
    deleteExpense: "Delete expense",
    editOrder: "Edit order",
    deleteOrder: "Delete order",
    editProduct: "Edit product",
    deleteProduct: "Delete product",
    dashboard: "Dashboard",
    products: "Products",
    orders: "Orders",
    expenses: "Expenses",
    help: "Help",
    meetDeveloper: "Meet the Developer",
    openSidebar: "Open sidebar",
    closeSidebar: "Close sidebar",

    summary: "Summary",
    analytics: "Analytics",
    revenue: "Revenue",
    balance: "Balance",
    ordersCount: "Orders",
    totalSales: "Total Sales",
    totalSalesDollars: "Total Sales ($)",
    salesByCategory: "Sales by Product Category",
    expensesChartDesc: "Donut chart showing expense totals grouped by transaction category, based on saved expense data.",
    salesChartDesc: "Bar chart showing total sales grouped by product category, based on saved product data.",

    usingGuide: "Using BizTrack: A Quick Guide",
    whatIsBizTrack: "What is BizTrack?",
    whatIsBizTrackText: "BizTrack is your go-to business management tool designed with small business owners in mind. It's an all-in-one platform that helps you effortlessly manage your products, track orders, and stay on top of your finances. Let me walk you through the basics:",
    navigatingDashboard: "Navigating the Dashboard",
    navigatingDashboardText: "The Dashboard is your central hub, giving you a snapshot of your business's overall performance. Here, you'll find key metrics like total expenses, revenues, profits and the number of orders. It's your command center for a quick overview.",
    expensesPage: "Expenses Page",
    recordExpensesText: "Record Your Expenses: Head to the Expenses page to add your business expenses. Fill in the date, choose a category, enter the amount, and jot down any notes. It's that simple.",
    editDeleteExpensesText: "Edit or Delete Expenses: Made a mistake? You can easily edit or delete expense records right from the Expenses page.",
    ordersPage: "Orders Page",
    trackOrdersText: "Track Your Orders: On the Orders page, you can keep tabs on all your orders. Each entry details the product, quantity, and order status.",
    effortlessEditingText: "Effortless Editing: Need to update an order status? Click the Edit button and make your changes.",
    addingNewEntry: "Adding a New Expense, Order or Product",
    addingNewEntryStep1: "Click Add Expense or the equivalent button on the order or product page.",
    addingNewEntryStep2: "Fill in the product details, order details, or expense details.",
    addingNewEntryStep3: "Submit the form and the new record will appear on the relevant page and dashboard.",
    sortingSearching: "Sorting and Searching Entries/Tables",
    sortingSearchingStep1: "Click a table heading to sort records by that column.",
    sortingSearchingStep2: "Use the search box at the top of each page to find matching records.",
    exportToCSVTitle: "Export to CSV",
    exportToCSVText: "Click Export to CSV to download a backup of your business data.",
    helpClosingText: "BizTrack is designed to be intuitive, user-friendly, and adaptable to your business needs.",
    contactPrompt: "Have questions, feedback, or want to connect? Feel free to reach out!",

    addOrder: "Add Order",
    exportCSV: "Export to CSV",
    downloadCSV: "Download CSV",
    orderID: "Order ID:",
    orderIDShort: "Order ID",
    orderDate: "Order Date:",
    orderDateShort: "Order Date",
    orderDatePlaceholder: "YYYY-MM-DD",
    itemName: "Item Name:",
    itemNameShort: "Item Name",
    itemPrice: "Item Price:",
    itemPriceShort: "Item Price",
    quantityBought: "Quantity Bought:",
    shippingFee: "Shipping Fee:",
    shippingFeeShort: "Shipping Fee",
    taxes: "Taxes (VAT/GST/HST):",
    taxesShort: "Taxes",
    orderStatus: "Order Status:",
    orderStatusShort: "Order Status",
    totalOrderAmount: "Total Order Amount:",
    orderTotalShort: "Order Total",
    calculated: "(Calculated)",
    chooseItem: "Choose an item",
    chooseStatus: "Choose a status",
    pending: "Pending",
    processing: "Processing",
    shipped: "Shipped",
    delivered: "Delivered",
    totalRevenue: "Total Revenue",

    addProduct: "Add Product",
    productID: "Product ID:",
    productIDShort: "Product ID",
    productName: "Product Name",
    productDescription: "Product Description:",
    productCategory: "Product Category:",
    productPrice: "Product Price:",
    unitsSold: "Number of Units Sold:",
    chooseProduct: "Choose a product",
    chooseCategory: "Choose a category",
    description: "Description",
    category: "Category",
    price: "Price",
    unitsSoldShort: "Units Sold",

    hats: "Hats",
    drinkware: "Drinkware",
    clothing: "Clothing",
    accessories: "Accessories",
    homeDecor: "Home decor",
    baseballCaps: "Baseball caps",
    snapbacks: "Snapbacks",
    beanies: "Beanies",
    bucketHats: "Bucket hats",
    mugs: "Mugs",
    waterBottles: "Water bottles",
    tumblers: "Tumblers",
    tshirts: "T-shirts",
    sweatshirts: "Sweatshirts",
    hoodies: "Hoodies",
    pillowCases: "Pillow cases",
    toteBags: "Tote bags",
    stickers: "Stickers",
    posters: "Posters",
    framedPosters: "Framed posters",
    canvasPrints: "Canvas prints",
    peaceEmbroideredCap: "Peace embroidered cap",
    floralLotusPrintedBottle: "Floral lotus printed bottle",
    palestineSweater: "Palestine sweater",
    vibesPrintedPoster: "Vibes printed poster",
    morrocanPrintPillowCase: "Morrocan print pillow case",

    search: "Search",
    addExpense: "Add Expense",
    date: "Date:",
    dateShort: "Date",
    expenseCategory: "Expense Category",
    categoryLabel: "Category:",
    chooseExpenseCategory: "Choose an expense category",
    amount: "Amount:",
    amountShort: "Amount",
    notes: "Notes:",
    notesShort: "Notes",
    serialNumber: "S/N",
    totalExpenses: "Total Expenses",
    rent: "Rent",
    orderFulfillment: "Order Fulfillment",
    utilities: "Utilities",
    supplies: "Supplies",
    miscellaneous: "Miscellaneous",

    action: "Action",
    add: "Add",
    update: "Update",
    cancel: "Cancel",
    qty: "Qty",
    codingJourney: "My Coding Journey",
    languageToggle: "中文",
    switchToChinese: "Switch language to Chinese",
    switchToEnglish: "Switch language to English",

    aboutIntroTitle: "Hey, I'm Sumayyah!",
    aboutIntroText: "Welcome to my little corner of the internet, where I'm rolling up my sleeves and diving into the coding world. I'm not your typical tech guru; I'm just a small business owner navigating entrepreneurship and learning to code.",
    aboutBizTrackOrigin: "BizTrack was born from the need to keep track of products, orders, and finances in one place.",
    aboutBizTrackPurpose: "BizTrack is my attempt at making business management more straightforward for small business owners.",
    aboutLearningJourney: "I'm a student at GetCoding NL, and BizTrack is my first module project.",
    aboutCoachThanks: "I am grateful for the guidance of my coding coach, Sam Russell.",
    aboutInvitation: "Whether you're into small business life, coding, or both, you're welcome here.",
    aboutClosing: "Here's to coding, business, and everything in between.",

    cookieBannerTitle: "Your privacy matters",
    cookieBannerText: "We use cookies and local storage to improve your experience, remember your preferences, and understand how BizTrack is used. You can accept all cookies, reject non-essential cookies, or manage your privacy choices.",
    cookieRejectAll: "Reject All",
    cookieManagePreferences: "Privacy Management",
    cookieAcceptAll: "Accept All",
    cookiePrivacyLink: "Privacy Policy",

    cookiePreferencesTitle: "Privacy Settings",
    cookiePreferencesText: "Choose which types of cookies or local storage BizTrack can use. Necessary storage is required for the website to work and cannot be disabled.",
    cookieNecessaryTitle: "Necessary Storage",
    cookieNecessaryText: "Required for core features such as page navigation, language support, and basic security.",
    cookieAlwaysOn: "Always On",
    cookiePreferenceTitle: "Preference Storage",
    cookiePreferenceText: "Remember choices such as language and interface preferences.",
    cookieAnalyticsTitle: "Analytics Cookies",
    cookieAnalyticsText: "Help us understand how the system is used so we can improve the experience.",
    cookieSavePreferences: "Save Preferences",
    cookieClosePreferences: "Close privacy settings",
    cookieManagePreferences: "Privacy Management",

    privacyPolicyTitle: "Privacy Policy",
    privacyPolicyIntro: "BizTrack is a small business management prototype for managing products, orders, and expenses. This policy explains how this prototype uses local data, cookies, and browser storage.",
    privacyPolicyDataTitle: "Information we may use",
    privacyPolicyDataText: "BizTrack may use information entered by users, such as product records, order details, expense information, and interface preferences. This coursework prototype is not intended for storing sensitive or confidential business data.",
    privacyPolicyCookiesTitle: "Cookies and local storage",
    privacyPolicyCookiesText: "BizTrack uses browser local storage to remember language preferences and privacy choices. Optional analytics cookies are only enabled if the user agrees.",
    privacyPolicyChoicesTitle: "Your choices",
    privacyPolicyChoicesText: "You can accept all cookies, reject non-essential cookies, or customise your privacy preferences. You can reopen Privacy Management at any time from the bottom-right button.",
    privacyPolicySecurityTitle: "Security notice",
    privacyPolicySecurityText: "BizTrack follows reasonable prototype design practices, but this version is for educational demonstration and should not be treated as a production legal or security solution.",
    privacyPolicyClose: "Close privacy policy"
      },

  zh: {
    primaryNavigation: "主导航",
    viewRevenueDetails: "在订单页面查看收入明细",
    viewExpenseDetails: "在支出页面查看支出明细",
    viewOrdersDetails: "在订单页面查看订单",
    currentBalanceSummary: "当前余额概览",
    visitLinkedIn: "访问 Sumayyah 的 LinkedIn 主页",
    visitGithub: "访问 Sumayyah 的 GitHub 主页",
    sendEmail: "发送邮件给 Sumayyah",
    editExpense: "编辑支出",
    deleteExpense: "删除支出",
    editOrder: "编辑订单",
    deleteOrder: "删除订单",
    editProduct: "编辑产品",
    deleteProduct: "删除产品",
    dashboard: "仪表盘",
    products: "产品",
    orders: "订单",
    expenses: "支出",
    help: "帮助",
    meetDeveloper: "开发者介绍",
    openSidebar: "打开侧边栏",
    closeSidebar: "关闭侧边栏",

    summary: "概览",
    analytics: "数据分析",
    revenue: "收入",
    balance: "余额",
    ordersCount: "订单",
    totalSales: "总销售额",
    totalSalesDollars: "总销售额（$）",
    salesByCategory: "按产品类别统计销售额",
    expensesChartDesc: "甜甜圈图显示按支出类别分组的总额，数据来自已保存的支出记录。",
    salesChartDesc: "柱状图显示按产品类别分组的总销售额，数据来自已保存的产品数据。",

    usingGuide: "BizTrack 使用指南",
    whatIsBizTrack: "什么是 BizTrack？",
    whatIsBizTrackText: "BizTrack 是一款面向小型企业主的业务管理工具。它将产品管理、订单跟踪和财务管理整合在一个平台中，帮助用户更轻松地管理日常业务。",
    navigatingDashboard: "浏览仪表盘",
    navigatingDashboardText: "仪表盘是系统的核心页面，可以快速展示总支出、收入、利润和订单数量等关键指标。",
    expensesPage: "支出页面",
    recordExpensesText: "记录支出：进入支出页面添加业务支出，填写日期、类别、金额和备注即可。",
    editDeleteExpensesText: "编辑或删除支出：如果记录有误，可以直接在支出页面编辑或删除对应记录。",
    ordersPage: "订单页面",
    trackOrdersText: "跟踪订单：订单页面会列出所有订单，并显示产品、数量和订单状态等信息。",
    effortlessEditingText: "便捷编辑：需要更新订单状态时，点击编辑按钮并修改内容即可。",
    addingNewEntry: "添加新的支出、订单或产品",
    addingNewEntryStep1: "点击添加按钮进入对应表单。",
    addingNewEntryStep2: "填写产品、订单或支出信息。",
    addingNewEntryStep3: "提交后，新记录会显示在对应页面并同步到仪表盘。",
    sortingSearching: "排序和搜索记录",
    sortingSearchingStep1: "点击表格列标题，可以按该列排序。",
    sortingSearchingStep2: "使用页面顶部搜索框可以查找匹配记录。",
    exportToCSVTitle: "导出 CSV",
    exportToCSVText: "点击导出 CSV 可以下载业务数据备份。",
    helpClosingText: "BizTrack 的目标是直观、易用，并适应日常业务管理需求。",
    contactPrompt: "如果你有问题、反馈或想进一步联系，欢迎随时联系我们！",

    addOrder: "添加订单",
    exportCSV: "导出 CSV",
    downloadCSV: "下载 CSV",
    orderID: "订单编号：",
    orderIDShort: "订单编号",
    orderDate: "订单日期：",
    orderDateShort: "订单日期",
    orderDatePlaceholder: "请选择日期",
    itemName: "商品名称：",
    itemNameShort: "商品名称",
    itemPrice: "商品价格：",
    itemPriceShort: "商品价格",
    quantityBought: "购买数量：",
    shippingFee: "运费：",
    shippingFeeShort: "运费",
    taxes: "税费（VAT/GST/HST）：",
    taxesShort: "税费",
    orderStatus: "订单状态：",
    orderStatusShort: "订单状态",
    totalOrderAmount: "订单总金额：",
    orderTotalShort: "订单总额",
    calculated: "（自动计算）",
    chooseItem: "请选择商品",
    chooseStatus: "请选择状态",
    pending: "待处理",
    processing: "处理中",
    shipped: "已发货",
    delivered: "已送达",
    totalRevenue: "总收入",

    addProduct: "添加产品",
    productID: "产品编号：",
    productIDShort: "产品编号",
    productName: "产品名称",
    productDescription: "产品描述：",
    productCategory: "产品类别：",
    productPrice: "产品价格：",
    unitsSold: "已售数量：",
    chooseProduct: "请选择产品",
    chooseCategory: "请选择类别",
    description: "描述",
    category: "类别",
    price: "价格",
    unitsSoldShort: "已售数量",

    hats: "帽子",
    drinkware: "饮具",
    clothing: "服装",
    accessories: "配饰",
    homeDecor: "家居装饰",
    baseballCaps: "棒球帽",
    snapbacks: "平檐帽",
    beanies: "针织帽",
    bucketHats: "渔夫帽",
    mugs: "马克杯",
    waterBottles: "水瓶",
    tumblers: "随行杯",
    tshirts: "T 恤",
    sweatshirts: "卫衣",
    hoodies: "连帽衫",
    pillowCases: "枕套",
    toteBags: "帆布袋",
    stickers: "贴纸",
    posters: "海报",
    framedPosters: "带框海报",
    canvasPrints: "帆布画",
    peaceEmbroideredCap: "和平刺绣帽",
    floralLotusPrintedBottle: "花卉莲花印花水瓶",
    palestineSweater: "巴勒斯坦主题毛衣",
    vibesPrintedPoster: "氛围感印花海报",
    morrocanPrintPillowCase: "摩洛哥印花枕套",

    search: "搜索",
    addExpense: "添加支出",
    date: "日期：",
    dateShort: "日期",
    expenseCategory: "支出类别",
    categoryLabel: "类别：",
    chooseExpenseCategory: "请选择支出类别",
    amount: "金额：",
    amountShort: "金额",
    notes: "备注：",
    notesShort: "备注",
    serialNumber: "序号",
    totalExpenses: "总支出",
    rent: "租金",
    orderFulfillment: "订单履约",
    utilities: "公用事业",
    supplies: "用品",
    miscellaneous: "其他",

    action: "操作",
    add: "添加",
    update: "更新",
    cancel: "取消",
    qty: "数量",
    codingJourney: "我的编程旅程",
    languageToggle: "EN",
    switchToChinese: "切换语言为中文",
    switchToEnglish: "切换语言为英文",

    aboutIntroTitle: "你好，我是 Sumayyah！",
    aboutIntroText: "欢迎来到我的网络角落。在这里，我正在认真学习编程。我不是传统意义上的技术专家，而是一名正在创业中学习如何用代码解决问题的小企业主。",
    aboutBizTrackOrigin: "BizTrack 来自一个真实需求：我需要在一个地方管理产品、订单和财务。",
    aboutBizTrackPurpose: "BizTrack 是我希望让小企业管理变得更简单的一次尝试。",
    aboutLearningJourney: "我是 GetCoding NL 的学生，BizTrack 是我的第一个模块项目。",
    aboutCoachThanks: "感谢我的编程导师 Sam Russell 在学习过程中的指导。",
    aboutInvitation: "无论你关注小企业经营、编程学习，还是两者的结合，这里都欢迎你。",
    aboutClosing: "献给编程、业务，以及两者之间的一切。",

    cookieBannerTitle: "您的隐私很重要",
    cookieBannerText: "我们使用 Cookie 和本地存储来改善您的使用体验、记住偏好设置，并了解 BizTrack 的使用情况。您可以接受所有 Cookie、拒绝非必要 Cookie，或管理您的隐私选择。",
    cookieRejectAll: "全部拒绝",
    cookieManagePreferences: "隐私设置",
    cookieAcceptAll: "全部接受",
    cookiePrivacyLink: "隐私政策",

    cookiePreferencesTitle: "隐私设置",
    cookiePreferencesText: "请选择 BizTrack 可以使用的 Cookie 或本地存储类型。必需存储用于网站正常运行，不能关闭。",
    cookieNecessaryTitle: "必需存储",
    cookieNecessaryText: "用于页面导航、语言支持和基础安全等核心功能。",
    cookieAlwaysOn: "始终开启",
    cookiePreferenceTitle: "偏好存储",
    cookiePreferenceText: "用于记住语言选择和界面偏好等设置。",
    cookieAnalyticsTitle: "分析 Cookie",
    cookieAnalyticsText: "帮助我们了解系统使用情况，从而改进用户体验。",
    cookieSavePreferences: "保存偏好",
    cookieClosePreferences: "关闭隐私设置",
    cookieSettingsButton: "隐私管理",

    privacyPolicyTitle: "隐私政策",
    privacyPolicyIntro: "BizTrack 是一个用于管理产品、订单和支出的中小型业务管理原型。本政策说明该原型如何使用本地数据、Cookie 和浏览器存储。",
    privacyPolicyDataTitle: "我们可能使用的信息",
    privacyPolicyDataText: "BizTrack 可能会使用用户输入的信息，例如产品记录、订单详情、支出信息和界面偏好。本课程项目原型不应用于存储敏感或机密的业务数据。",
    privacyPolicyCookiesTitle: "Cookie 与本地存储",
    privacyPolicyCookiesText: "BizTrack 使用浏览器本地存储来记住语言偏好和隐私选择。可选的分析 Cookie 只有在用户同意后才会启用。",
    privacyPolicyChoicesTitle: "您的选择",
    privacyPolicyChoicesText: "您可以接受所有 Cookie、拒绝非必要 Cookie，或自定义隐私偏好。您之后也可以通过右下角的隐私管理按钮重新打开设置。",
    privacyPolicySecurityTitle: "安全说明",
    privacyPolicySecurityText: "BizTrack 会尽量遵循合理的原型设计实践，但当前版本仅用于课程展示，不应被视为正式生产环境下的法律或安全解决方案。",
    privacyPolicyClose: "关闭隐私政策"
  }
};

const valueTranslationKeys = {
  "Hats": "hats",
  "Drinkware": "drinkware",
  "Clothing": "clothing",
  "Accessories": "accessories",
  "Home decor": "homeDecor",
  "Baseball caps": "baseballCaps",
  "Snapbacks": "snapbacks",
  "Beanies": "beanies",
  "Bucket hats": "bucketHats",
  "Mugs": "mugs",
  "Water bottles": "waterBottles",
  "Tumblers": "tumblers",
  "T-shirts": "tshirts",
  "Sweatshirts": "sweatshirts",
  "Hoodies": "hoodies",
  "Pillow cases": "pillowCases",
  "Tote bags": "toteBags",
  "Stickers": "stickers",
  "Posters": "posters",
  "Framed posters": "framedPosters",
  "Canvas prints": "canvasPrints",
  "Peace embroidered cap": "peaceEmbroideredCap",
  "Floral lotus printed bottle": "floralLotusPrintedBottle",
  "Palestine sweater": "palestineSweater",
  "Vibes printed poster": "vibesPrintedPoster",
  "Morrocan print pillow case": "morrocanPrintPillowCase"
};

function getCurrentLanguage() {
  const language = localStorage.getItem("bizTrackLanguage") || "en";
  return translations[language] ? language : "en";
}

function getText(key) {
  const language = getCurrentLanguage();
  return translations[language][key] || translations.en[key] || key;
}

function translateValue(value) {
  const key = valueTranslationKeys[value];
  return key ? getText(key) : value;
}

function applyTranslatedText(element, translatedText) {
  const hasOnlyOneBoldElement =
    element.children.length === 1 &&
    element.children[0].tagName === "B";

  const hasExtraTextOutsideBold = Array.from(element.childNodes).some((node) => {
    return node.nodeType === Node.TEXT_NODE && node.textContent.trim() !== "";
  });

  if (hasOnlyOneBoldElement && !hasExtraTextOutsideBold) {
    const boldText = element.children[0];
    const smallText = boldText.querySelector("small");

    if (smallText) {
      boldText.replaceChildren(document.createTextNode(`${translatedText} `), smallText);
      smallText.textContent = getText("calculated");
    } else {
      boldText.textContent = translatedText;
    }

    return;
  }

  element.textContent = translatedText;
}

function setLanguage(language) {
  const normalizedLanguage = translations[language] ? language : "en";
  localStorage.setItem("bizTrackLanguage", normalizedLanguage);
  document.documentElement.lang = normalizedLanguage === "zh" ? "zh-CN" : "en";

  document.querySelectorAll("[data-i18n]").forEach((element) => {
    applyTranslatedText(element, getText(element.getAttribute("data-i18n")));
  });

  document.querySelectorAll("[data-i18n-label]").forEach((element) => {
    element.setAttribute("label", getText(element.getAttribute("data-i18n-label")));
  });

  document.querySelectorAll("[data-i18n-aria-label]").forEach((element) => {
    element.setAttribute("aria-label", getText(element.getAttribute("data-i18n-aria-label")));
  });

  document.querySelectorAll("[data-i18n-placeholder]").forEach((element) => {
    element.setAttribute("placeholder", getText(element.getAttribute("data-i18n-placeholder")));
  });

  const toggleButton = document.getElementById("language-toggle");
  if (toggleButton) {
    toggleButton.textContent = getText("languageToggle");
    toggleButton.setAttribute(
      "aria-label",
      normalizedLanguage === "en" ? getText("switchToChinese") : getText("switchToEnglish")
    );
  }

  document.dispatchEvent(new CustomEvent("languageChanged"));
}

function toggleLanguage() {
  setLanguage(getCurrentLanguage() === "en" ? "zh" : "en");
}

document.addEventListener("DOMContentLoaded", () => {
  setLanguage(getCurrentLanguage());
});

function safeCSVValue(value) {
  var str = String(value);
  if (str === "") return '""';
  if (str.indexOf(",") !== -1 || str.indexOf('"') !== -1 || str.indexOf("\n") !== -1 || str.indexOf("\r") !== -1) {
    return '"' + str.replace(/"/g, '""') + '"';
  }
  if (/^[=+\-@]/.test(str)) {
    return '"' + str.replace(/"/g, '""') + '"';
  }
  return str;
}

function safeGenerateCSV(data) {
  if (!data || data.length === 0) return "";
  var headers = Object.keys(data[0]).map(safeCSVValue).join(",");
  var rows = data.map(function (row) {
    return Object.values(row).map(safeCSVValue).join(",");
  });
  return headers + "\n" + rows.join("\n");
}